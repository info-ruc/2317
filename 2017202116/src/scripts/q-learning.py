...
for episode in range(num_episodes):
    ...
    for t in range(max_number_of_steps):
        ...
        score+= 1
        if done:
            print('%d Episode finished after %f score steps / mean %f' % (episode,score, last_time_steps.mean()))
            last_time_steps = np.hstack((last_time_steps[1:], [score]))  # 更新最近100场游戏的得分stack
            break
            # 如果最近100场平均得分高于195
